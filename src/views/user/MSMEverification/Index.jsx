...
import { msme_verification } from '@utils/Filed'
...
const Index = () => {
  ...
  const initialValues = msme_verification[service_name]?.fileds.reduce((values, field) => {
    values[field.name] = '';
    return values;
  }, {});

  // dynamic valiadtion schema
  const validationSchema = msme_verification[service_name]?.fileds.reduce((schema, field) => {
    if (field.require) {
      ...
      <div className="max-w-8xl mx-auto bg-white shadow-md rounded-lg">
        <div className="py-6 px-8">
          <h3 className="text-lg font-medium text-gray-700 mb-4 border-b">
            {msme_verification[service_name]?.formLabel}
          </h3>
          <Formik
            ...
            onSubmit={(values) => {
              ...
              setSpin(true)
              API.post(msme_verification[service_name]?.APIURL, values)
                .then((response) => {
          ...
          >
            {() => (
              <Form>
                {
                  msme_verification[service_name]?.fileds?.map((value, index) => {
                    return (
                      <>

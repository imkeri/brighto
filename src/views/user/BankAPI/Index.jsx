...
const Index = () => {
  ...
  const initialValues = bank[service_name]?.fileds.reduce((values, field) => {
    values[field.name] = '';
    return values;
  }, {});

  // dynamic valiadtion schema
  const validationSchema = bank[service_name]?.fileds.reduce((schema, field) => {
    if (field.require) {
        ...
        <div className="py-6 px-8">
          ...
          <Formik
            ...
            onSubmit={(values) => {
              ...
              setSpin(true)
              API.post(bank[service_name]?.APIURL, values)
                .then((response) => {
          ...
          >
            {() => (
              <Form>
                {
                  bank[service_name]?.fileds?.map((value, index) => {
                    return (
                      <>

...
import { vehicle_related_apis } from '@utils/Filed'
...
const Index = () => {
  ...
  const initialValues = vehicle_related_apis[service_name]?.fileds.reduce((values, field) => {
    values[field.name] = '';
    return values;
  }, {});

  // dynamic valiadtion schema
  const validationSchema = vehicle_related_apis[service_name]?.fileds.reduce((schema, field) => {
    if (field.require) {
      if (field?.name === "name" || field?.name === "state" || field?.name === "client_name") {
        ...
        schema[field.name] = Yup.string()
          .matches(/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[0-2])\/\d{4}$/, 'Enter valid date in dd/mm/yyyy format')
          .test('isValidDate', 'Enter a valid date in dd/mm/yyyy format', value => {
    ...
    <>
      ...
      <div className="max-w-8xl mx-auto bg-white shadow-md rounded-lg">
        <div className="py-6 px-8">
          <h3 className="text-lg  text-customGray font-bold mb-4 border-b">
            {vehicle_related_apis[service_name]?.formLabel}
          </h3>
          <Formik
            ...
            onSubmit={(values) => {
              ...
              setSpin(true)
              API.post(vehicle_related_apis[service_name]?.APIURL, values)
                .then((response) => {
          ...
          >
            {() => (
              <Form>
                {
                  vehicle_related_apis[service_name]?.fileds?.map((value, index) => {
                    return (
                      <>

...
import { epfo_apis } from "@utils/Filed";
...
const Index = () => {
  ...
  const initialValues = epfo_apis[service_name]?.fileds.reduce(
    (values, field) => {
      values[field.name] = "";
      return values;
    },
  ...
  const validationSchema = epfo_apis[service_name]?.fileds.reduce(
    (schema, field) => {
      if (field.require) {
        if (field?.name === "name" || field?.name === "state" || field?.name === "client_name") {
          ...
          schema[field.name] = Yup.string().matches(/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i, 'Enter valid email').required(`${field.label} is required`).required(`${field.label} is required`);
        ...
        if (field.name === "email") {
          schema[field.name] = Yup.string().matches(/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i, 'Enter valid email').email("Invalid email format");
        } else if (field.name === "phone") {
      ...
      <div className="max-w-8xl mx-auto bg-white shadow-md rounded-lg">
        <div className="py-6 px-8">
          <h3 className="text-lg font-medium text-gray-700 mb-4 border-b">
            {epfo_apis[service_name]?.formLabel}
          </h3>
          <Formik
            ...
            initialValues={{
              plan_id: id,
              user_id: userAuth?.id,
              services: service_name.toLowerCase().replace(/ /g, "_"),
              ...initialValues,
            }}
            onSubmit={(values) => {
              ...
              setSpin(true);
              API.post(epfo_apis[service_name]?.APIURL, values)
                .then((response) => {
                  ...
                  if (response?.code === 200) {
                    ...
                    const obj = {
                      ...
                      "Date&Time": moment(res?.created_at).format(
                        "ddd MMM DD YYYY HH:mm:ss"
                      ),
                      RatePerCheck: response?.data?.transaction?.price,
                    };
                    const cleanedString = response?.data?.data?.response?.slice(
                      1,
                      -1
                    );
          ...
          >
            {() => (
              <Form>
                {epfo_apis[service_name]?.fileds?.map((value, index) => {
                  return (
                    <>
                      <div className="m-2" key={index}>
                        <label
                          htmlFor={value?.name}
                          className="block text-sm font-medium text-customGray-700"
                        >
                          {value?.label}
                        </label>
                        <Field
                ...
                <div className="mt-4 mx-2">
                  <button
                    type="submit"
                    className=" py-2 px-4 bg-[#f19357] text-white rounded-md hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-offset-2"
                  >
                    Submit
                  </button>
                </div>
          ...
          {data && (
            <div className="my-3">
              ...
              <div className="flex items-center justify-between p-3">
                <button onClick={() => downloadPDF(service_name,setSpin)}>
                  <MdDownload />
                </button>
                <button onClick={() => handleCopyClick(data)}>
                  <MdContentCopy />
                </button>
              </div>
              <Table striped hover>
